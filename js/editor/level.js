...
var Level = function() {
	...
	self.Undo = function() {
		if(local.undo.length != 0) {
			...
			var old_tile = self.GetDataXY(info.un_x, info.un_y);
			
			if( (info.un_mode == DRAW_MODE_ROUND) || (info.un_mode == DRAW_MODE_SINGLE)) {
				Draw(info.un_x, info.un_y, info.un_tile, DRAW_MODE_SINGLE);
			} else if(info.un_mode == DRAW_MODE_LINE) {
				
				for(var i = info.un_left; i <= info.un_right; i++) {
					Draw(i, info.un_y, info.un_tile, DRAW_MODE_SINGLE);
				}
			}
	...
	self.Redo = function() {
		if(local.redo.length != 0) {
			...
			if(info.un_mode == DRAW_MODE_ROUND) {
				...
				Draw(info.un_x, info.un_y, info.un_tile, info.un_mode);
			} else if(info.un_mode == DRAW_MODE_LINE) {
